...
write_sero_pars(alpha=c(0.03,0.03), fname=sero_file)
write_res_pars(p_tau=0.4,cost_res=COST,tau=TAU,fname=res_file)
write_sc_pars(kappa=c(1.1, 1.1),fname=sc_file)
## Run
res1_hightreat <- run_simulations(sc_file, sero_file, res_file, niter=no_cores*3, label="(a)")%>%
  mutate(treat="High Treatment")
write_res_pars(p_tau=0.1,cost_res=COST,tau=TAU,fname_res=res_file)
## Run
res1_lowtreat <- run_simulations(sc_file, sero_file, res_file,niter=no_cores*3,label="(a)")%>%
...
write_sero_pars(alpha=c(0.03,0.03), fname=sero_file)
write_res_pars(p_tau=0.4,cost_res=COST,tau=TAU,fname=res_file)
write_sc_pars(kappa=c(1.1),fname=sc_file)
## Run
res2_hightreat <- run_simulations(sc_file, sero_file, res_file,niter=no_cores*3, label="(b)")%>%
  mutate(treat="High Treatment")

write_res_pars(p_tau=0.1,cost_res=COST,tau=TAU,fname=res_file)
## Run
res2_lowtreat <- run_simulations(sc_file, sero_file, res_file,niter=no_cores*3,label="(b)")%>%
  mutate(treat="Low Treatment")
...
write_sero_pars(alpha=c(0.03), fname=sero_file)
write_res_pars(p_tau=0.4,cost_res=COST,tau=TAU,fname=res_file)
write_sc_pars(kappa=c(1.1, 1.1),fname=sc_file)
## Run
res3_hightreat <- run_simulations(sc_file, sero_file, res_file, niter=no_cores*3, label="(c)")%>%
  mutate(treat="High Treatment")

write_res_pars(p_tau=0.1,cost_res=COST,tau=TAU,fname=res_file)
## Run
res3_lowtreat <- run_simulations(sc_file, sero_file, res_file, niter=no_cores*3,label="(c)")%>%
  mutate(treat="Low Treatment")
...
write_sc_pars(kappa=c(1.1),fname=sc_file)
## Run
res4_hightreat <- run_simulations(sc_file, sero_file, res_file, niter=no_cores*3, label="(d)")%>%
  mutate(treat="High Treatment")

write_res_pars(p_tau=0.1,cost_res=COST,tau=TAU,fname=res_file)
## Run
res4_lowtreat <- run_simulations(sc_file, sero_file, res_file, niter=no_cores*3,label="(d)") %>%
  mutate(treat="Low Treatment")
...
ggsave(paste0(out_dir, "/withinhost_illustraion.pdf"))
